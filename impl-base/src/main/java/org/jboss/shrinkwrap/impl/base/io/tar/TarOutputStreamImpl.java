...
public class TarOutputStreamImpl extends FilterOutputStream {
    ...
    /**
     * Ends the TAR archive without closing the underlying OutputStream. The result is that the EOF record of nulls is
     * written.
     */
    ...
    public void putNextEntry(TarEntry entry) throws IOException {
        ...
        if ((entry.isUnixTarFormat() && name.length() > TarHeader.NAMELEN)
            ...
            throw new InvalidHeaderException("file name '" + name + "' is too long ( " + name.length() + " > "
                + (entry.isUnixTarFormat() ? TarHeader.NAMELEN : (TarHeader.NAMELEN + TarHeader.PREFIXLEN))
                + " bytes )");
        }
